...
def inchToSm(value: int) -> float:
    """Функция inchToSm создана для быстрого преобразования Дюймов в
    Санатиметры. Достаточно передать в нее целое число"""
    return value * 2.54
def smToInch(value: int) -> float:
    """Функция smToInch создана для быстрого преобразования Санатиметров в
    Дюймы. Достаточно передать в нее целое число"""
    return value * 0.39
def mileToKm(value: int) -> float:
    """Функция mileToKm создана для быстрого преобразования Миль в
    Километры. Достаточно передать в нее целое число"""
    return value * 1.69
def kmToMile(value: int) -> float:
    """Функция kmToMile создана для быстрого преобразования Километров в
    Мили. Достаточно передать в нее целое число"""
    return value * 0.6214
def poundToKg(value: int) -> float:
    """Функция poundToKg создана для быстрого преобразования Фунтов в
    Килограммы. Достаточно передать в нее целое число"""
    return value * 0.454
def kgToPound(value: int) -> float:
    """Функция kgToPound создана для быстрого преобразования Килограмм в
    Фунты. Достаточно передать в нее целое число"""
    return value * 2.2
def ounceToGr(value: int) -> float:
    """Функция ounceToGr создана для быстрого преобразования Унций в
    Грраммы. Достаточно передать в нее целое число"""
    return value * 28.349
def grToOunce(value: int) -> float:
    """Функция grToOunce создана для быстрого преобразования Грамм в
    Унции. Достаточно передать в нее целое число"""
    return value * 0.0352739982605
def gallonToLiter(value: int) -> float:
    """Функция gallonToLiter создана для быстрого преобразования Галлонов в
    Литры. Достаточно передать в нее целое число"""
    return value * 3.785
def literToGallon(value: int) -> float:
    """Функция literToGallon создана для быстрого преобразования Литров в
    Галлоны. Достаточно передать в нее целое число"""
    return value * 0.26417
def pintToLiter(value: int) -> float:
    """Функция pintToLiter создана для быстрого преобразования Пинт в
    Литры. Достаточно передать в нее целое число"""
    return value * 0,473176
def literToPint(value: int) -> float:
    """Функция literToPint создана для быстрого преобразования Литров в
    Пинты. Достаточно передать в нее целое число"""
...
def Main():
    """Функция Main создана для проверки работоспособности функций приведеных
    выше.

    Функция при активации выведет список возможных операций и запросит целое
    число для выбора необходимой для конвертции единицы измерения.
    Далее достаточно передать ей целое число для  получения результата.
    Функция будет запрашивать у Вас выбор обперации бесконечное количество раз.
    Для того что бы проверка была остановлена, необходимо ввести 0.
    При попытке ввода номера операции, которого нет в списке, функция сообщит
    об ошибке и укажет дииапазон возможных для вводао перааций от 1 до 12."""
